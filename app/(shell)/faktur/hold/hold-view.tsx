...
export function HoldView() {
  ...
  async function save() {
    ...
    const payload = {
      ...
      no_sj: inv.no_sj.trim() || null, keterangan: ket.trim(),
    };
    const { error } = editing
  ...
  const shown = rows.filter((r) => `${r.invoice_no} ${r.bp_value} ${r.keterangan}`.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4">
      ...
      <div className={`${card} space-y-3 p-5`}>
        ...
        <textarea value={ket} onChange={(e) => setKet(e.target.value)} rows={2} placeholder="Keterangan *" className={inputCls} />
          ...
          <tbody>
            {shown.map((h) => (
              <tr key={h.id} className="border-t border-line">
                ...
                <td className={`${td} max-w-72 whitespace-normal`}>{h.keterangan}</td>
                <td className={`${td} text-right`}>
                  <button type="button" className="px-1 text-fg-2 hover:text-fg" title="Ubah"
                    onClick={() => { setEditing(h.id); setInv({ invoice_no: h.invoice_no, bp_value: h.bp_value, invoice_date: h.invoice_date ?? "", no_sj: h.no_sj ?? "" }); setKet(h.keterangan); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
